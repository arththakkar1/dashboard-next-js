...
const MOCK_RESULTS = [
  ...
  {
    id: 2,
    title: "Table 5 Reservation - 7:00 PM",
    type: "Reservation",
    href: "/reservations",
  },
...
export default function SearchModal({ isOpen, onClose }: SearchModalProps) {
  ...
  return createPortal(
    <AnimatePresence>
      {isOpen && (
        <motion.div
          className="fixed inset-0 z-100 flex items-start justify-center pt-20 sm:pt-24 px-4 bg-black/50 backdrop-blur-sm"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          />
          <motion.div
            className="relative w-full max-w-xl rounded-2xl border border-border bg-card shadow-2xl"
            role="dialog"
          ...
          >
            ...
            <div className="flex items-center border-b border-border px-4 py-3">
              ...
              <kbd
                className="hidden sm:inline-flex items-center justify-center rounded-md border border-border bg-secondary px-1.5 py-0.5 text-[10px] font-mono font-medium text-muted-foreground cursor-pointer"
                onClick={onClose}
              >
                ESC
              </kbd>
